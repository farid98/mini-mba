...
<Slide title="Strategy in Practice">

The offsite ritual: two days, SWOT analyses, aspirational vision statements, a list of priorities that is everything the company already does. Everyone returns Monday. Nothing changes.

This happens when strategy is treated as an **intellectual exercise** rather than a **decision-making process** — when the output is a document rather than binding choices with resource implications.

Three things real strategy in practice requires:

- Making choices under genuine uncertainty
- Communicating them in ways that actually change behaviour
- Knowing when to stay the course vs. when to adapt

</Slide>

<Slide title="Deliberate vs. Emergent Strategy">

**Deliberate strategy** — what we intend. The plan we set out to execute.

**Emergent strategy** — what actually happens. Patterns that develop from a thousand small field decisions.

Honda entered the U.S. motorcycle market planning to compete with Harley-Davidson on large bikes. That failed. What succeeded was the small Super Cub their own employees were riding to work — attracting a completely different customer segment.

**Realized strategy = deliberate plan + emergent patterns.**

*Design planning processes not just to execute a plan but to identify and exploit emergent opportunities as they appear.*

</Slide>

<Slide title="Planning That Works">

Five characteristics that distinguish effective strategic planning from the offsite ritual:

- **Starts with questions, not answers** — surface critical uncertainties before proposing solutions
- **Makes choices explicit** — including what you're choosing NOT to do
- **Allocates resources** — strategy without resource commitment is aspiration
- **Time horizon matched to the business** — semiconductor manufacturers plan 10 years; CPG companies plan 2–3
- **Includes triggers and review mechanisms** — what information would cause a review?

*If every initiative gets 10% more budget, the plan is not strategic — it's incrementalism.*

</Slide>

<Slide title="Decisions Under Uncertainty">

Four practical tools:

- **Scenario planning** — develop 2–4 distinct plausible futures; ask what strategy performs reasonably well across all of them
- **Real options thinking** — invest enough to learn whether an opportunity is real, then expand if evidence is positive
- **Pre-mortem** — "Imagine we executed this plan and it failed catastrophically. What happened?" Surfaces risks optimistic planning misses
- **Reversibility preference** — when two options have similar expected value, prefer the more reversible one

*Strategy is not about predicting the future correctly — it's about making robust choices that work across plausible futures.*

</Slide>

<Slide title="When to Pivot">

Three genuine signals to revisit a strategy:

- **The assumptions have changed** — not just challenged, but contradicted by evidence
- **Systematic execution failure** — persistent gaps below plan despite genuine effort, across multiple quarters
- **A structural inflection point** — the internet (2000), smartphones (2007), cloud (2010), generative AI (2023) each devalued previous advantages and opened new positions

**The hardest mistakes are made when things look fine.** Kodak, Blockbuster, and Nokia were all profitable when they needed to pivot. The companies that survived started adapting before financial pressure was visible.

</Slide>

<Slide title="Execution Depends on Leadership">

Strategy doesn't implement itself. Four binding constraints on execution:

- **Alignment at the top** — conflicting signals from senior leadership prevent the focused resource allocation strategy requires
- **Middle management translation** — the gap between strategic intent and operational action lives mostly here
- **Cultural compatibility** — strategies requiring behaviours at odds with existing culture will fail despite good analysis
- **Patience with compounding** — most strategic investments have long time horizons before returns are visible

*Amazon built Prime, AWS, and marketplace infrastructure for years before they dominated. Strategic leadership means protecting long-term investments from quarterly pressure.*
